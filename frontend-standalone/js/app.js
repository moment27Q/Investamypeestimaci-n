(() => {
  ...
  async function fetchUsoStatus() {
    try {
      ...
      let res;
      try {
        res = await fetch(apiUrl("/api/uso"), { signal: ctrl.signal, headers: { "Accept": "application/json" } });
      } finally {
        clearTimeout(timer);
      }
    ...
    try {
      ...
      try {
        res = window.Auth
          ...
          : await fetch(apiUrl("/api/uso/consumir"), { method: "POST", signal: ctrl.signal });
      } finally {
        clearTimeout(timer);
      }
      if (!res.ok) throw new Error("http");
    ...
    try {
      ...
      try {
        res = window.Auth
          ...
          : await fetch(apiUrl("/api/comparables?" + params.toString()), { signal: ctrl.signal });
      } finally {
        clearTimeout(timer);
      }
      const data = await res.json().catch(() => ({}));
    ...
    try {
      ...
      try {
        res = window.Auth
          ...
          : await fetch(apiUrl("/api/cocheras?" + params.toString()), { signal: ctrl.signal });
      } finally {
        clearTimeout(timer);
      }
      const data = await res.json().catch(() => ({}));
    ...
    try {
      ...
      try {
        res = window.Auth
          ...
          : await fetch(apiUrl("/api/rentals?" + params.toString()), { signal: ctrl.signal });
      } finally {
        clearTimeout(timer);
      }
      const data = await res.json().catch(() => ({}));
  ...
  async function fetchEnvironment() {
    ...
    try {
      ...
      const res = window.Auth
        ...
        : await fetch(apiUrl("/api/environment?" + params.toString()));
    ...
    try {
      ...
      try {
        res = window.Auth
          ...
          : await fetch(apiUrl("/api/descripcion"), {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              signal: ctrl.signal,
              body: JSON.stringify({
